...
var Utils = function () {
	...
	_createClass(Utils, [{
		...
		key: 'getHxBaselink',
		value: function getHxBaselink(svBaselink, isTier) {
			if (svBaselink) {
				if (!isTier) {
					var search = svBaselink.match(/https:\/\/base(0?[1-9]|[1-9][0-9])-sv/);
					if (search && search[0]) {
						var hxPrefix = search[0].replace("sv", "hx");
						return svBaselink.replace(search[0], hxPrefix);
					}
				} else {
					return svBaselink.replace("/s", "/h");
				}
			}
			return svBaselink;
		}
...
var Api = function () {
	...
	_createClass(Api, [{
		...
		value: function login(callback, data) {
			...
			chrome.storage.local.get([_ServiceMeta2.default.STORAGEKEYS.LANGUAGE, _ServiceMeta2.default.STORAGEKEYS.USERDATA, _ServiceMeta2.default.STORAGEKEYS.LSER], function (storage) {
				...
				_this.getOS().then(function (os) {
					...
					if (userdata && userdata.isHxLogin === true) data.isHx = true;
					if (data.isHx) parameter.brand = "hx";
					...
					_this.request(_endpoints2.default.LOGINAUTHCODE, 'POST', parameter, function (response, hxBaselink) {
						if (_Utils2.default.isResponse200(response) && response.config) {
							...
							// Set baselink and flag for hx login
							if (data.isHx && hxBaselink) {
								userdata.isHxLogin = true;
							}
							...
							if (_Utils2.default.isResponseAuthError(response) || _Utils2.default.isResponseMinusOne(response) || _Utils2.default.isMaintenanceMode(response) || _Utils2.default.isResponseMinus20(response)) {
								if (_Utils2.default.isResponseAuthError(response) && !data.isHx && data.hitReason == 'login') {
									var loginData = {
										hitReason: 'login',
									...
									_this.login(callback, loginData);
									return;
								}
								if (callback) callback(response);
								return;
		...
		key: 'request',
		value: function request(endpoint, method, parameter, callback, ignoreFail, isApi2, isHx) {
			var _this = this;
			...
			chrome.storage.local.get([_ServiceMeta2.default.STORAGEKEYS.BASELINK, _ServiceMeta2.default.STORAGEKEYS.BASELINKTYPE, _ServiceMeta2.default.STORAGEKEYS.TIMEOUTSETTINGS, _ServiceMeta2.default.STORAGEKEYS.USERDATA], function (storage) {
				...
				var userdata = storage[_ServiceMeta2.default.STORAGEKEYS.USERDATA];
				//fix brand if user isHxLogin in storage
				if (userdata && userdata.isHxLogin === true && parameter.brand) {
					parameter.brand = "hx";
				}

				var Baselink = storage[_ServiceMeta2.default.STORAGEKEYS.BASELINK];
				// req to hx baselink if isHxLogin or first login hx
				if (isHx || userdata && userdata.isHxLogin === true) {
					Baselink = _Utils2.default.getHxBaselink(storage[_ServiceMeta2.default.STORAGEKEYS.BASELINK], isTier);
				}
				var xhr = new XMLHttpRequest();
				...
				xhr.onreadystatechange = function () {
					if (xhr.readyState == 4) {
						if (xhr.status == 200) {
							...
							return callback(result, Baselink);
						} else {
...
var _MigrationAddon = __webpack_require__(28);

var _MigrationAddon2 = _interopRequireDefault(_MigrationAddon);
		...
		value: function onInstalled(details) {
			if (details && details.reason) {
				...
				if (details.reason == 'install') {
					this.setInstallId();
					_MigrationAddon2.default.getState().then(function (state) {
						if (state && (typeof state === 'string' || state instanceof String)) {
							var d = {
								...
								if (storage[_ServiceMeta2.default.STORAGEKEYS.USERDATA] === undefined) {
									chrome.storage.local.set(_defineProperty({}, _ServiceMeta2.default.STORAGEKEYS.USERDATA, d));
								}
							});
						}
					}).catch(function () {});
	...
	_createClass(ProxyApi, [{
		...
		value: function getRulesForFixedServer(proxy) {
			...
			return new Promise(function (resolve, reject) {
				...
				chrome.storage.local.get([_ServiceMeta2.default.STORAGEKEYS.BASELINK, _ServiceMeta2.default.STORAGEKEYS.BYPASSLIST, _ServiceMeta2.default.STORAGEKEYS.CONFIGDATA, _ServiceMeta2.default.STORAGEKEYS.BASELINKTYPE, _ServiceMeta2.default.STORAGEKEYS.USERDATA], function (storage) {
					...
					var currentBaselinkProtocolStripped = baselink.replace("http://", "").replace("https://", "");
					...
					// Add hx baselink to bypasslist
					var userData = storage[_ServiceMeta2.default.STORAGEKEYS.USERDATA];
					if (userData && userData.isHxLogin === true) {
						var isTier = storage[_ServiceMeta2.default.STORAGEKEYS.BASELINKTYPE] === "tier" ? true : false;
						if (!isTier) {
							var hxBaselink = _Utils2.default.getHxBaselink(baselink, isTier);
							var hxBaselinkProtocalStripped = hxBaselink.replace("http://", "").replace("https://", "");
							var curHxBaselink = hxBaselinkProtocalStripped.substr(0, hxBaselinkProtocalStripped.indexOf('/'));
							if (curHxBaselink == "") curHxBaselink = hxBaselinkProtocalStripped;
							curHxBaselink = _Utils2.default.removePort(curHxBaselink);
							curHxBaselink = curHxBaselink.toLowerCase();
							byPassList.push(curHxBaselink);
						}
					}
...
var _localhosts = __webpack_require__(17);
...
var MigrationAddon = function () {
	function MigrationAddon() {
		_classCallCheck(this, MigrationAddon);

		this.addonPort = false;
		this.addonId = "nbcojefnccbanplpoffopkoepjmhgdgh";
		this.requests = {};
	}

	_createClass(MigrationAddon, [{
		key: 'getState',
	...
	return MigrationAddon;
}();

exports.default = new MigrationAddon();
...
exports.sha512 = __webpack_require__(19)
...
var inherits = __webpack_require__(7)
var SHA512 = __webpack_require__(19)
var Hash = __webpack_require__(8)
var Buffer = __webpack_require__(4).Buffer
